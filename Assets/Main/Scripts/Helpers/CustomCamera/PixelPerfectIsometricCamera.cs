...
namespace Main.Scripts.Helpers.CustomCamera
{
    /// <summary>
    ...
    [AddComponentMenu("Rendering/2D/Pixel Perfect Isometric Camera")]
    ...
    public class PixelPerfectIsometricCamera : MonoBehaviour, IPixelPerfectCamera, ISerializationCallbackReceiver
    {
        /// <summary>
        ...
        public Vector3 RoundToPixel(Vector3 position)
        {
            ...
            Vector3 result;
            var verticalUnitsPerPixel = unitsPerPixel / (float)Math.Sqrt(2);
            result.x = Mathf.Round(position.x / unitsPerPixel) * unitsPerPixel;
            result.y = Mathf.Round(position.y / verticalUnitsPerPixel) * verticalUnitsPerPixel;
            result.z = Mathf.Round(position.z / verticalUnitsPerPixel) * verticalUnitsPerPixel;
        ...
        void PixelSnap()
        {
            ...
            var scaleMatrix = Matrix4x4.Scale(new Vector3(1.0f, (float)Math.Sqrt(2.0f), -1.0f));
        ...
        void OnBeginCameraRendering(ScriptableRenderContext context, UnityEngine.Camera camera)
        {
            if (camera == m_Camera)
            {
                UpdateCameraProperties();
        ...
        void OnEndCameraRendering(ScriptableRenderContext context, UnityEngine.Camera camera)
        {
            if (camera == m_Camera)
